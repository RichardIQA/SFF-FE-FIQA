...
def distillation_loss(output, labels, teacher_output, teacher_features, student_features, T, alpha):
    """
    计算知识蒸馏的损失函数
    :param output: 学生模型的输出
    :param labels: 真实标签
    :param teacher_output: 教师模型的输出
    :param teacher_features: 教师模型的中间特征层
    :param student_features: 学生模型的中间特征层
    ...
    for teacher_feature, student_feature in zip(teacher_features, student_features):
        feature_loss += nn.MSELoss()(student_feature / T, teacher_feature / T)

    # 总损失
    total_loss = alpha * student_loss + (1 - alpha) * teacher_loss + 0.01 * feature_loss
    return total_loss
...
def main(config):
    ...
    teacher_model = AIGCModel.FIQA()
    student_model = AIGCModel.mobileNetV3()
    ...
    # optimizer
    optimizer = optim.Adam(student_model.parameters(), lr=config.conv_base_lr, weight_decay=0.000001)
    ...
    trainset = ImageDataset_VQC(config.train_dir, config.train_datainfo, transformations_train, config.resize, config.frames, 'train')
    valset = ImageDataset_VQC(config.val_dir, config.val_datainfo, transformations_train, config.resize, config.frames, 'val')
    ...
    for epoch in range(config.epochs):
        ...
        for i, (video, rs, mos, name) in enumerate(train_loader):
            ...
            with torch.no_grad():
                teacher_pred, teacher_features = teacher_model(video, rs)
            student_pred, student_features = student_model(video, rs)
            loss = distillation_loss(student_pred, labels, teacher_pred, teacher_features, student_features, T=2, alpha=0.5)
        ...
        with torch.no_grad():
            ...
            for i, (video, rs, mos, name) in enumerate(val_loader):
                ...
                rs = rs.to(device)
                label[i] = mos.item()
                outputs, _ = student_model(video, rs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_dir', type=str, default='FIQA/train')
    parser.add_argument('--train_datainfo', type=str, default='data/train.csv')

    parser.add_argument('--val_dir', type=str, default='FIQA/train')
    ...
    parser.add_argument('--frames', type=int, default=5)  # frames
    ...
    parser.add_argument('--train_batch_size', type=int, default=128)  # batch
    ...
    parser.add_argument('--Save_Path', type=str, default='Distillation')
    parser.add_argument('--pretrained_weights_path', type=str, default='FIQA.pth')
