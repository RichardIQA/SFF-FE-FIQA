...
class MBWTConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1, bias=True, wt_levels=1, wt_type='db1',
        ...
        self.global_atten1 = SS2D(d_model=in_channels, d_state=1,
                                  ssm_ratio=ssm_ratio, initialize="v2", forward_type=forward_type, channel_first=True,
                                  k_group=2)
        self.channel_atten = HighFreqEnhancer(in_channels)
        self.base_scale = _ScaleModule([1, in_channels, 1, 1])
        ...
        self.after_conv = nn.Conv2d(in_channels, in_channels, kernel_size, padding='same', stride=1, dilation=1,
    ...
    def forward(self, x):
        ...
        for i in range(self.wt_levels - 1, -1, -1):
            ...
            curr_x_h = curr_x_h.permute(0, 2, 1, 3, 4)
            curr_x_h = self.channel_atten(curr_x_h)
...
class FIQAMambaModuleV0(torch.nn.Module):
    def __init__(self, dim, kernels=3, ssm_ratio=1, forward_type="v052d", ):
        super().__init__()
        self.dim = dim
...
class FIQAMambaBlockWindow(torch.nn.Module):
    def __init__(self, dim, global_ratio=0.25, local_ratio=0.25,
        ...
        super().__init__()
        self.dim = dim
        self.attn = FIQAMambaModule(dim, global_ratio=global_ratio, local_ratio=local_ratio,
...
class FIQAMambaBlock(torch.nn.Module):
    def __init__(self, type,
        ...
        if type == 's':
            self.mixer = Residual(FIQAMambaBlockWindow(ed, global_ratio=global_ratio, local_ratio=local_ratio,
...
class FIQAMamba(torch.nn.Module):
        ...
        for i, (stg, ed, dpth, gr, lr, do) in enumerate(
            ...
            for d in range(dpth):
                eval('self.blocks' + str(i + 1)).append(
                    FIQAMambaBlock(stg, ed, gr, lr, kernels[i], dpr[d], ssm_ratio=ssm_ratio,
                                   forward_type=forward_type))
            if do[0] == 'subsample':
                # Build FIQAMamba downsample block
                # ('Subsample' stride)
