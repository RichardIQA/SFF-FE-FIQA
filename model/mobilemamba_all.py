...
class MBWTConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1, bias=True, wt_levels=1, wt_type='db1',
        ...
        self.cross_attention = CrossAttention(embed_size=self.in_channels, heads=4)
    ...
    def forward(self, x):
        ...
        for i in range(self.wt_levels - 1, -1, -1):
            ...
            curr_x_h = self.global_atten1(curr_x_h)
...
class MobileMambaModule_all(torch.nn.Module):
    def __init__(self, dim, embed_dim, kernels=3, ssm_ratio=1, forward_type="v052d", ):
        super().__init__()
        self.dim = dim
        # self.attn = nn.MultiheadAttention(embed_dim, num_heads=7)
        # self.attn1 = nn.MultiheadAttention(embed_dim, num_heads=7)
...
class MobileMambaBlockWindow_all(torch.nn.Module):
    def __init__(self, dim, embed_dim, kernels=5, ssm_ratio=1, forward_type="v052d", ):
        super().__init__()
        self.dim = dim
        self.attn = MobileMambaModule_all(dim, embed_dim, kernels=kernels, ssm_ratio=ssm_ratio,
...
class MobileMambaBlock_all(torch.nn.Module):
    def __init__(self, ed, embed_dim, kernels=5, drop_path=0., has_skip=True, ssm_ratio=1, forward_type="v052d"):
        super().__init__()

        self.dw0 = Residual(Conv2d_BN(ed, ed, 3, 1, 1, groups=ed, bn_weight_init=0.))
        self.ffn0 = Residual(FFN(ed, int(ed * 2)))

        self.mixer = Residual(MobileMambaBlockWindow_all(ed, embed_dim, kernels=kernels, ssm_ratio=ssm_ratio,
                                                     forward_type=forward_type))
...
class MobileMambaModule(torch.nn.Module):
    def __init__(self, dim, embed_dim, global_ratio=0.5, local_ratio=0.5,
        ...
        self.conbine_block = CombinedConvBlock(in_channels=self.local_channels, out_channels=self.local_channels)

    def forward(self, x):  # x (B,C,H,W)
        x1, x2 = torch.split(x, [self.global_channels, self.local_channels], dim=1)
        size = x.shape
        x1 = self.global_op(x1)
        x2 = self.conbine_block(self.local_op(x2))
        attn_output, attn_weights = self.attn(x1.reshape(size[0], size[1] // 2, -1),
                                              x2.reshape(size[0], size[1] // 2, -1),
                                              x2.reshape(size[0], size[1] // 2, -1))
        # print(attn_output.shape)
        attn_output = attn_output.reshape(size[0], size[1] // 2, size[2], size[3])
        attn_output1, attn_weights1 = self.attn1(x2.reshape(size[0], size[1] // 2, -1),
                                                 x1.reshape(size[0], size[1] // 2, -1),
                                                 x1.reshape(size[0], size[1] // 2, -1))
        attn_output1 = attn_output1.reshape(size[0], size[1] // 2, size[2], size[3])
        x = self.proj(torch.cat([attn_output + x1, attn_output1 + x2], dim=1))
        # print(x3.shape)
...
class MobileMambaBlockWindow(torch.nn.Module):
    def __init__(self, dim, embed_dim, global_ratio=0.25, local_ratio=0.25,
                 kernels=5, ssm_ratio=1, forward_type="v052d", ):
        super().__init__()
        self.dim = dim
        self.attn = MobileMambaModule(dim, embed_dim, global_ratio=global_ratio, local_ratio=local_ratio,
...
class MobileMambaBlock(torch.nn.Module):
    def __init__(self, ed, embed_dim, global_ratio=0.5, local_ratio=0.5,
        ...
        self.mixer = Residual(MobileMambaBlockWindow(ed, embed_dim, global_ratio=global_ratio, local_ratio=local_ratio,
